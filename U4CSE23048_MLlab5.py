...
def task_A2_evaluate_single(model, X_train, X_test, y_train, y_test):
    ...
    y_test_pred = model.predict(X_test)
    metrics = {}
    for name, (true, pred) in {"train": (y_train, y_train_pred), "test": (y_test, y_test_pred)}.items():
        mse = mean_squared_error(true, pred)
        rmse = np.sqrt(mse)
        mape = mean_absolute_percentage_error(true, pred)
        r2 = r2_score(true, pred)
...
def task_A3_all_features():
    ...
    for col in feature_cols:
        corr = df[col].corr(df["clarity_score"])
        if abs(corr) > 0.999:
            leak_cols.append(col)
    if leak_cols:
        print(f"⚠ Removing possible leakage columns: {leak_cols}")
